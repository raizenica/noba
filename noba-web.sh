...
# Try ports from 8080 to 8090 until one is free
START_PORT="${1:-8080}"
MAX_PORT=8090
PORT=""
for try_port in $(seq "$START_PORT" "$MAX_PORT"); do
    if ! ss -tuln | grep -q ":$try_port "; then
        PORT="$try_port"
        break
    fi
done
...
# Function to find and kill any process using a port (if we later need to free a specific one)
kill_port_process() {
    local port=$1
    local pid=""
...
# Clean up old processes (ignore port; they might be on a different port)
kill_pid_file "$SERVER_PID_FILE" "server"
kill_pid_file "$UPDATER_PID_FILE" "updater"

# We don't need to kill port process because we already found a free port

# Generate the main HTML page (same as before)
...
# Create the Python server (with broken pipe protection)
cd "$HTML_DIR" || exit 1
cat > server.py <<'EOF'
import http.server
import socketserver
