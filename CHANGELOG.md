...
### Changed
- **Per-rule autonomy enforcement (v3 Phase 4, Task 3)** — `evaluate_alert_rules()` now respects a per-rule `autonomy` field with four levels: `execute` (default, immediate action), `notify` (notification only, no action), `approve` (queues action in approval DB with `[APPROVAL NEEDED]` notification prefix), and `disabled` (rule skipped entirely). Active maintenance windows with `override_autonomy` override the rule-level setting. `_execute_heal()` delegates the six new action types (`flush_dns`, `clear_cache`, `trigger_backup`, `failover_dns`, `scale_container`, `run_playbook`) to `remediation.execute_action()` while keeping all legacy handlers unchanged. 27 new tests in `tests/test_autonomy.py`.
